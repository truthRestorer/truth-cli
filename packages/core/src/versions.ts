...
export function vControl(v: string) {
  if (v[0] === '^') return v.slice(0, v.indexOf('.'))
  if (v[0] === '~') return v.slice(0, v.lastIndexOf('.'))
  return v
}
...
export function genVersions(relations: Relations) {
  ...
  function loadVersions() {
    for (const [name, { dependencies = {}, devDependencies }] of Object.entries(
      relations,
    )) {
      for (const [pkgName, pkgVersion] of Object.entries(
        ...
        if (!pkgMap) {
          ...
          if (pkgMap[v] && !pkgMap[v].includes(name)) pkgMap[v].push(name)
          else pkgMap[v] = [name]
        }
      }
    }
