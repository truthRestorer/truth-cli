import { isEmptyObj, Relations } from '@truth-cli/shared'
...
export function genJson(
  ...
  for (const [name, version] of useEntries(devDependencies)) {
    pkgs.packages![name] = {
      version,
      type: PkgDependency.DEVDEPENDENCY,
      packages: {},
    }
  }
  ...
  for (const [name, version] of useEntries(dependencies)) {
    pkgs.packages![name] = {
      version,
      type: PkgDependency.DEPENDENCY,
      packages: {},
    }
  }

  if (!shouldOptimize) shouldOptimize = depth > 4
  // 递归(深度优先)产生 `pkgs.json` 内容数据
  function loadJson(rootPkgs: PkgJson, maxDep: number) {
    if (maxDep <= 0) {
      for (const key of Object.keys(rootPkgs)) delete rootPkgs[key].packages
      return
    }
    for (const key of Object.keys(rootPkgs)) {
      if (!rootPkgs[key].packages) continue
      const { dependencies, devDependencies } = relations[key] ?? {}
      pkgSet.add(key)
      for (const [name, version] of useEntries(dependencies)) {
        rootPkgs[key].packages[name] = {
          version,
          type: PkgDependency.DEPENDENCY,
        }
      ...
      if (isEmptyObj(rootPkgs[key].packages)) delete rootPkgs[key].packages
      else loadJson(rootPkgs[key].packages, maxDep - 1)
